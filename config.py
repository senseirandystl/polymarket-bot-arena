...
def _load_repo_dotenv() -> None:
    """Load repo `.env` into os.environ for native (non-Compose) runs.

    Does not override variables already set in the process environment.
    Missing file is a no-op. Compose already injects env; this mirrors that
    for `python arena.py` / `dashboard/server.py` on Windows/macOS/Linux.
    """
...
# Paper-only gate profile (Pass A data-gather). Live mode ignores these and
# uses the base constants below. Set PAPER_GATE_PROFILE=off (or empty) to
# disable even in paper. data_gather_v1 = cautious loosen for fill collection.
PAPER_GATE_PROFILE = (os.environ.get("PAPER_GATE_PROFILE") or "off").strip()
PAPER_GATE_OVERRIDES = {
    "data_gather_v1": {
        # Dual-gate: loosen Z only; keep moneyness PCT floor.
        "DRIFT_MIN_ABS_Z": 0.28,
        "KALSHI_DRIFT_MIN_ABS_Z": 0.28,
        "MODEL_LEAN_MIN": 0.04,
        "MIN_EDGE_DEFAULT": 0.010,
        "ARBITRAGE_MIN_MARGIN": 0.015,
        "SWEEPER_MIN_IMPLIED": 0.965,
    },
}


# Optional Pass-B / data-gather loosen for bot DEFAULT_PARAMS when
# paper_gates_active(). Live and paper-with-profile-off keep profit-tight bases.
PAPER_BOT_PARAM_OVERLAYS = {
    "data_gather_v1": {
        "sniper": {"min_drift": 0.12, "min_edge": 0.016},
        "sweeper": {
            "entry_window_sec": 100,
            "min_drift": 0.28,
            "min_twap_certainty": 0.40,
        },
        "arbitrage": {"min_margin": 0.015},
    },
}


def apply_paper_bot_params(strategy_type: str, params: dict) -> dict:
    """Return params with optional paper-profile overlay (copy-safe).

    Base defaults stay profit-tight. Only when ``paper_gates_active()`` and the
    active profile lists the strategy do we loosen selected keys.
    """
    out = dict(params or {})
    if not paper_gates_active():
        return out
    profile = (PAPER_GATE_PROFILE or "").strip().lower()
    ov = (PAPER_BOT_PARAM_OVERLAYS.get(profile) or {}).get(strategy_type) or {}
    for k, v in ov.items():
        out[k] = v
    return out


def paper_gates_active() -> bool:
    """True when paper mode and a known PAPER_GATE_PROFILE is selected."""
    profile = (PAPER_GATE_PROFILE or "").strip().lower()
    if profile in ("", "off", "none", "0", "false"):
        return False
    if TRADING_MODE != "paper":
        return False
    return profile in PAPER_GATE_OVERRIDES


def effective_float(name: str, default=None) -> float:
    """Config float with optional paper-profile override (live always base).

    DB gate-tuner overrides are applied separately in arena.gate_tuner.gate_float.
    """
    if default is None:
        default = float(globals()[name])
    base = float(default)
    if not paper_gates_active():
        return base
    profile = PAPER_GATE_PROFILE.strip().lower()
    ov = PAPER_GATE_OVERRIDES.get(profile) or {}
    if name in ov:
        return float(ov[name])
    return base


def paper_gate_snapshot() -> dict:
    """Ops/dashboard view of the active paper gate profile.

    Returns profile name, active flag, and a small overrides-vs-base map for
    the keys operators care about (z, lean, min_edge, arb margin, sweeper).
    """
    profile = (PAPER_GATE_PROFILE or "").strip()
    active = paper_gates_active()
    # Friendly short labels for the Overview chip tooltip / subtitle.
    key_map = (
        ("DRIFT_MIN_ABS_Z", "z"),
        ("MODEL_LEAN_MIN", "lean"),
        ("MIN_EDGE_DEFAULT", "min_edge"),
        ("ARBITRAGE_MIN_MARGIN", "arb_margin"),
        ("SWEEPER_MIN_IMPLIED", "sweeper_implied"),
    )
    overrides: dict = {}
    ov = {}
    if active:
        ov = PAPER_GATE_OVERRIDES.get(profile.strip().lower()) or {}
    for name, label in key_map:
        base = float(globals().get(name))
        effective = float(ov[name]) if name in ov else base
        if name in ov or active:
            overrides[label] = {
                "key": name,
                "base": base,
                "effective": effective,
                "overridden": name in ov,
            }
    reason = None
    if TRADING_MODE != "paper":
        reason = "live_mode"
    elif (profile or "").strip().lower() in ("", "off", "none", "0", "false"):
        reason = "profile_off"
    elif not active:
...
# Legacy plaintext location â€” the active source of truth is the encrypted
...
# midpoint reads â€” a retry-sleep there would stall the trader tick, and those
# calls are already best-effort with a warm-cache fallback. Worst-case added
# latency per call â‰ˆ backoff_baseÂ·(2^0 + 2^1) â‰ˆ 1.2s at the defaults.
...
# when the operator has never saved. Off â‡’ no discovery/eval/fills for that
...
# 15m Ïƒ seed (~âˆš3 vs 5m prior if the process is similar). Soak-tune later.
KALSHI_DRIFT_VOL_SCALE = 0.0038
KALSHI_DRIFT_MIN_ABS_PCT = 0.00080
KALSHI_DRIFT_MIN_ABS_Z = 0.35  # live/base; paper data_gather_v1 -> 0.28 via effective_float
...
# ---------------------------------------------------------------------------
# Strategy Lab pipeline (invent -> gate -> graduate).
# Default enabled with PAPER_SLOTS=0 so invent/backtest runs without deploy.
# ---------------------------------------------------------------------------
STRATEGY_LAB_ENABLED = True
STRATEGY_LAB_CYCLE_INTERVAL_SEC = 300.0
STRATEGY_LAB_MAX_OPEN = 8
STRATEGY_LAB_MAX_NEW_PER_TICK = 2
STRATEGY_LAB_PAPER_SLOTS = 0
STRATEGY_LAB_AUTO_PROMOTE = False
STRATEGY_LAB_BACKTEST_MIN_PNL = 1.0
STRATEGY_LAB_BACKTEST_MIN_TRADES = 5
STRATEGY_LAB_LLM_PROVIDER = "none"
STRATEGY_LAB_PROMOTE_MIN_TRADES = 100
STRATEGY_LAB_PROMOTE_MIN_DAYS = 7
STRATEGY_LAB_PROMOTE_TRADE_FLOOR = 30
...
# Database â€” override with ARENA_DB_PATH for Docker / non-default layouts.
...
# by PAPER_MAX_POSITION â€” it now only scales the maker/arb position_size_pct
# knobs (via get_max_position()).
PAPER_MAX_POSITION = 50.0  # $SIM sizing base for maker/arb bots
# Paper daily-loss floors (aligned with RISK_PAPER_*). Was 999999
# "uncapped" which left the risk-engine-off legacy path unprotected.
PAPER_MAX_DAILY_LOSS_PER_BOT = 75.0
PAPER_MAX_DAILY_LOSS_TOTAL = 150.0
# PAPER_STARTING_BALANCE removed (was 10000 unused); paper capital is
# PAPER_BANKROLL_DEFAULT / dashboard arena_state 'paper_bankroll'.
...
# Directed / GA default gene-flip probability (BUG #31 â†’ full GA 2026-07-23):
...
# Mirrors default slate size (arena.startup.DEFAULT_INDICES).
NUM_BOTS = 3
...
EVOLUTION_WINDOW_HOURS = 72        # 24â†’72 (audit: 24h overfits current regime; 72h covers multiple regime shifts)
# Raised 15 -> 30 (2026-07-21): a 5-min-market window of 15-20 resolved trades
# is dominated by noise â€” cycle 6 killed sniper-v1 on a 17-trade / -$8.49 dip
...
# Raised 30 â†’ 40 (2026-08 soak): GA culled meanrev/mom/sniper/phantom on
...
# Post-TWAP trade rate can sit at 1â€“13 fills / 72h. Adaptive floor lets the
...
# can still be replaceable when P&L and BE gap are catastrophically bad â€”
...
# Paper Pass A: raise early-cull min trades so thin samples survive longer.
PAPER_GA_EARLY_CULL_MIN_TRADES = int(
    os.environ.get("PAPER_GA_EARLY_CULL_MIN_TRADES", "30") or 30
)
...
# 2026-08: meanrev-v1 at âˆ’$3 was replaced by a worse mutant.
EVOLUTION_PNL_CULL_MAX = -12.0  # only replaceable if window PnL â‰¤ this
...
GA_FOUNDER_CULL_PNL = -20.0     # gen0 replaceable only if pnl â‰¤ this
GA_FOUNDER_CULL_BE_GAP = -0.02  # or BE gap worse than âˆ’2Â¢ with enough n
...
# vol/direction bounced across normalâ†”low_vol_rangeâ†”low_vol_trend every few
# minutes and portfolio rebalanced on every flip. 20 ticks â‰ˆ 20s hold.
...
# Never UP a lane when strategyÃ—regime P&L is negative with enough fills.
# Regime-local cells are thin overnight (n=3â€“8 trades) while attribution n is
# large â€” use a lower bar so red $ blocks UP before accuracy alone pumps drift.
...
# Accuracy is a one-way UP veto (must clear UP_ACC_FLOOR). Missing EV â†’ HOLD,
# never an accuracy-led UP (soak 2026-08-24: mom 0.20â†’0.40 on 56.9% WR, âˆ’1.45Â¢).
CORE_TUNE_EV_PRIMARY = True
CORE_TUNE_EV_MIN_TRADES = 20
CORE_TUNE_EV_UP_MIN = 0.0          # need mean attributed $ â‰¥ this to UP
CORE_TUNE_EV_DOWN_MAX = -0.05      # mean attributed $ â‰¤ this â†’ DOWN
...
# Unique-market scorecard net Â¢/share (preferred judge over sign accuracy).
CORE_TUNE_SCORECARD_MIN = 20
CORE_TUNE_SCORECARD_DOWN_MAX = 0.0   # net â‰¤ this â†’ block UP
CORE_TUNE_SCORECARD_FORCE_DOWN = -0.005  # net â‰¤ this â†’ step DOWN
CORE_TUNE_SCORECARD_MAX_ENTRY = 0.62  # ignore 84Â¢+ dual-gate rows in overlay
...
# Continuous residual w = w0 + BÂ·F (off until sample mass)
...
# Strategy routing (capital / GA) â€” not hard-skip
...
# DEPRECATED (Phase 1 audit): no live code path reads these; kept for
# dashboard/regime_settings UI keys. Do not re-enable without wiring.
REGIME_FREQ_TARGET_ENABLED = False
REGIME_FREQ_TARGET_FILLS_PER_HOUR = 4.0
REGIME_FREQ_EDGE_EASE_MAX = 0.15
# Live strategyÃ—regime style-skip (data-driven; default ON after 2026-08-06 soak)
...
# Fast path: overnight cells are thin â€” n=6 at WRâ‰¤38% + red $ is enough to
...
# Side-aware continuous tax / binary side-skip (strategyÃ—regimeÃ—side)
...
# Lower bar so overnight strategyÃ—regime toxicity raises min_edge / cuts size
...
GA_PERF_TRIGGER_PNL = -25.0    # fire early if pool window P&L â‰¤ this
...
# High stickiness (2026-08): cross-type swaps (phantomâ†’sentiment) destroyed
...
GA_SPAWN_EXCLUDE_TYPES = ()       # filled dynamically if empty â€” see type_alloc
...
# n=2 is noise â€” sniper-v1 sat in the bank at 50% WR / -$0.46 with n=2).
GA_GENE_BANK_MIN_TRADES = 5
# When True, prune bank entries that have â‰¥ MIN_TRADES and pnl < 0 so a once-
...
# --- Sniper ask quality ---
# Max (ask âˆ’ mid) on the chosen side; wider spreads mean the lag thesis on
...
# Total capital budget shared by ALL not-ready / cold bots (prevents 3Ã—24%).
PORTFOLIO_EXPLORE_TOTAL_BUDGET = 0.12
# Proven bots (long-window exp>0 or gen0 with nâ‰¥floor) keep at least this
...
# --- Learned trade rules (decision_events â†’ auto skip/go) ---
# Mines regimeÃ—priceÃ—driftÃ—side cells; promotes skip when buys lose and go
...
# Never ease (GO) high-price cells from skip counterfactuals alone â€” that
# re-opened expensive favorites (2026-08: â‰¥0.72 mid âˆ’$38). Require real buy
...
# 1 - (yes_ask + no_ask + fees) per share regardless of outcome â€” market-neutral.
...
ARBITRAGE_MIN_MARGIN = 0.025    # fee-aware; survives ~1s latency (was 0.02)
...
# Â¢) a BUY's realized avg fill price may exceed the price the decision expected
# before the fill is REJECTED (reason "slippage_exceeded"). This kills the class
# of loss where a razor-thin edge (esp. the arbitrage bot's ~1-2Â¢/pair) is wiped
...
# late-window book (overnight: 5â€“10 rejects/bot/window). Maker section (~20s)
...
# measured anti-predictive AGAIN (confirms-side WR 22% vs contradicts 50%) â€” the
...
# OBI lane for every strategy â€” keep 0.0 until a fade-sign OBI is validated
# offline.
...
# (harness p50 0.126, p97 0.40) â€” it was pegged at +/-1.000 on 79% of the
...
# follow-WR) but NET edge NEGATIVE (-0.80c/share at avg entry 0.688) â€” the
# market has already priced its own momentum by the time it is measurable.
# House rule: no positive net edge, no live weight. Same treatment as OBI â€”
...
# one-sided â€” sign(recent tape), the same magnitude disease as pm_mom. Live
# ground truth: cvd-driven trades (|cvd| >= 0.8, |drift| < 0.10) ran 53.1% WR
# (+$10.47 over 32 trades) â€” statistically flat, no net edge. The feed now
...
# alone â€” one candle of quiet-tape noise is not a trend. When the volatility
...
# signal â€” it is the bot's analyze() thesis, usually derived from the same
...
# Modes:
#   confirm â€” only contribute when sign agrees with drift; magnitude scaled
#   residual â€” always allow but scale down (legacy-ish)
#   full â€” old additive behavior (not recommended)
...
# been offline-validated the way drift/mom were â€” it was assumed reasonable
...
# 0.6: 36.1% WR, -$60.15 over 36 trades â€” the single worst bucket in the run;
...
# Stand down only when TWAP is actually locked (honest tanh ~0.70 â‰ˆ Î¦ 0.85).
# 0.40 was a catch-22 with dual-gate zâ‰¥0.35: meanrev never had a fade window.
MEANREV_MIN_FADE_DRIFT = 0.70

# --- NO-side intelligence (2026-08 soak: YES +$245 / NO âˆ’$15) ---
...
NO_SIDE_STRATEGY_EDGE_MULT = {
    ...
    "no_lag": 1.0,              # NO specialist â€” already NO-only
    "true_maker": 1.20,
}

# --- Cheap underdog band (0.35â€“0.42): mild leak at 38% WR / âˆ’$23 ---
UNDERDOG_BAND_LO = 0.35
UNDERDOG_BAND_HI = 0.42
UNDERDOG_MIN_DRIFT = 0.18
UNDERDOG_EDGE_MULT = 1.40
# Residual-lag check in the mid band, scored with Î¦(z) not tanh.
...
MAKER_MAX_MID_ASK_GAP = 0.08        # |mid âˆ’ ask| above this â†’ refuse trade
...
# --- Candidate signal lanes (2026-07-18) â€” ALL kill-switched at 0 ---
...
# for each (house rule: validate-before-weighting â€” see BUG #23/#26/#27 for
...
SIGNAL_WEIGHT_XASSET = 1.0   # ETH/SOL confirm-only (live +0.51Â¢/share, 79% WR)
...
# --- Expanded candidate lanes (2026-08 audit) â€” kill-switched until Lab ---
...
SIGNAL_WEIGHT_LAG = 0.0      # market-lag residual: moneyness-implied P âˆ’ mid
...
LAG_MONEYNESS_SCALE = 0.0015   # ~0.15% moneyness â†’ ~0.76
# Spread context (non-directional): widen min_edge when book is wide.
SPREAD_EDGE_MULT_ENABLED = True
SPREAD_EDGE_WIDE = 0.04      # spread fraction of mid above this â†’ tax
...
# philosophy as the session filter â€” "build the skip, default flat").
MACRO_CAUTION_SKIP = 0.75

# --- Live lane monitor (arena/lane_monitor.py) â€” the DEMOTION half of the
...
# 74-80% follow-WR â€” live it scored 51.7% over 209 trades (harness numbers
...
LANE_MONITOR_MIN_NET_EDGE = 0.0     # â‰¤ 0Â¢/share after fee â†’ demote at full sample
...
# the sparser lanes (fut_oi) only collect ~300-360 samples per 300 markets â€”
...
# dashboard approval (Signal Lab) â€” this only automates the measurement.
...
# --- Auto-approve promoter (arena/lane_promoter.py) â€” closed loop ---
...
# kill-switch) clear a LIVE bar over a real resolved sample â€” never on the
...
# must earn promotion by a clearer margin than it needs to merely survive â€”
...
AUTO_APPROVE_MIN_ACCURACY = 0.57  # 0.55â†’0.57 (audit: wider 4pp gap vs demotion 0.53 for hysteresis)
# Require positive LIVE shadow net edge (Â¢/share after fee) in addition to
# accuracy â€” accuracy alone promoted fut at ~55% that later scored ~38% live.
AUTO_APPROVE_MIN_NET_EDGE = 0.005  # +0.5Â¢/share on shadow follow-the-sign
...
# 80%â†’âˆ’5.5Â¢ incident). Below this fill count the verdict stays "collecting".
AUTO_APPROVE_MIN_FILLS = 15
AUTO_APPROVE_MAX_ACTIVE = 3       # cap on simultaneously-enabled CANDIDATE lanes
# Core-lane tuner apply toggle â€” SEPARATE from auto-approve so operators can
...
# --- Core-lane auto-tuner (arena/core_lane_tuner.py) â€” the loop's core half ---
# The candidate-lane loop above tunes fut/tech/xasset (which feed a few bots at
# ~0.10 weight). This tunes the lanes that drive EVERY directional trade â€”
# drift/mom/strat â€” PER strategy, on that strategy's own live attribution
...
# can run away or collapse (drift especially â€” the one validated lane), a real
...
# --- Regime discovery & conditioning (Layer 3 â€” arena/regime_map.py) ---
...
# above â€” OFF means the map is still built and reported, but no downstream
...
# Sentiment feed master switch (2026-07-18): OFF â€” no local LLM will be run
...
# Recal 2026-08-07: 0.0015 was too tight at ~$65k TWAP tape â€” small late-window
# wiggles printed as |drift|â‰¥0.3 and directional follow WR collapsed. Prior
# raised; adaptive Ïƒ prefers **TWAP** samples (same object as moneyness).
DRIFT_VOL_SCALE = 0.0022          # ~0.22% of price full-window prior
DRIFT_ADAPTIVE_SCALE = True       # False â†’ always use DRIFT_VOL_SCALE prior
DRIFT_VOL_SCALE_MIN = 0.0018      # 0.0010 mapped 5bp TWAP wiggles to |d|â‰ˆ0.55
...
# Prefer Chainlink TWAP tick series for adaptive Ïƒ (falls back to spot 1m).
DRIFT_ADAPT_USE_TWAP = True
# Sample near the TWAP lookback so increments are not overlapping 5s
# averages (those pin Ïƒ at the floor and invent 78Â¢ implied probs).
DRIFT_ADAPT_TWAP_SAMPLE_SEC = 60.0
# Raw moneyness floor (fraction): dual-gate with z-score so $noise â‰  "strong drift".
# Overnight 2026-08-20: 5â€“6 bp at Ïƒ floor 0.0018 cleared |z|â‰¥0.35 and printed
# Î¦â‰ˆ0.64â€“0.69 on 32â€“38Â¢ underdogs (core directional 12/35 = 34% WR).
# 8 bp â‰ˆ $55 at $70k â€” still below a real 5m move; binds when z is inflated.
DRIFT_MIN_ABS_PCT = 0.00080
# Sniper: 8bp dual-gate is enough inside the 50â€“58Â¢ lag pocket (soak +EV);
...
# Momentum sits out the last ~80s on 5m (soak: late<=80s was -$EV).
# Prefer fraction-of-window so 5m/15m stay comparable; absolute SEC
# values remain as fallbacks when window_sec is missing.
MOMENTUM_LATE_SKIP_FRAC = 80 / 300  # ~0.2667 of window
MOMENTUM_LATE_SKIP_SEC = 80  # fallback / PM 5m absolute
SNIPER_MIN_AGE_FRAC = 60 / 300  # 0.20 of window (PM 5m ~60s)
SNIPER_MIN_AGE_SEC = 60  # fallback absolute when window unknown
# Meanrev: retrace of TWAP toward this window's strike, not a 4-bar z-score.
MEANREV_PULLBACK_MIN = 0.20
# Floor remaining-window time in Ïƒ_rem = scaleÂ·âˆš(tr/W) so last-minute noise
# cannot explode z. 60s â‡’ Ïƒ_rem never below scaleÂ·âˆš(60/300).
DRIFT_TIME_SCALE_MIN_SEC = 60.0
# Dual-gate bar on *raw* z = moneyness / Ïƒ_rem (not tanh). 0.35 â‰ˆ 6â€“8 bp
# mid-window at Ïƒ=0.22%. Pre-TWAP-recal this was compared to tanh(z).
DRIFT_MIN_ABS_Z = 0.35
# Sweeper lock floor on tanh(z). 0.32 is only a first filter â€” a 99Â¢ buy
# also needs Î¦(z) â‰¥ SWEEPER_MIN_IMPLIED (overnight tanh 0.455 @ 99Â¢ NO
# lost $5.70; Î¦ was ~0.67, not a lock).
...
# and final settlement are Chainlink **TWAP** values (60s for 5m markets) â€”
# not a single spot snapshot. See TWAP_* knobs below. Binance is NOT used live
# (basis ~$60â€“80 / ~0.1% vs Chainlink â€” enough to flip near-strike drift); if
...
#        @PolymarketDevs â€” both open PTB and settlement from TWAP feed.
# 5-min markets â†’ **60s TWAP** (was 30s at 2026-08-07 cutover; Polymarket
# announced lengthening the 5m lookback to 60s â€” override with TWAP_WINDOW_SEC).
...
# built from local ticks over [expiryâˆ’W, now] (fill remaining with last price).
TWAP_NOWCAST_ENABLED = True
# Minimum tick coverage of the settlement sub-window before trusting nowcast
# over the rolling RTDS TWAP alone (0â€“1).
TWAP_NOWCAST_MIN_COVERAGE = 0.40
# TWAP feed stale threshold (lookback windows â‰  publication cadence).
...
# true settlement â€” logged; prefer 0 drift if both missing).
TWAP_FALLBACK_TO_SPOT = True
# Soft vol damp for TWAP-vs-strike z-scores. Was 0.85 (made z *more* sensitive
# while Ïƒ was already from spot â€” double-counted smoothness). With TWAP-based
# adaptive Ïƒ, keep mult at 1.0; only damp if Ïƒ is still from spot fallback.
TWAP_DRIFT_VOL_MULT = 1.0
TWAP_DRIFT_VOL_MULT_SPOT_FALLBACK = 0.92  # mild damp only when Ïƒ from spot 1m
# --- Settlement-window policy (final TWAP_WINDOW_SEC of each market) -------
# Once rem â‰¤ TWAP_WINDOW_SEC the settlement TWAP is partially observed:
# certainty rises with elapsed fraction; last-tick spot spikes no longer flip
# the outcome. Policy reweights edge floors, size, mom damp â€” not a hard ban.
...
# Certainty thresholds (0â€“1 from twap_certainty())
TWAP_SETTLE_CERT_HIGH = 0.55      # "mostly locked" â€” ease edge, allow size
TWAP_SETTLE_CERT_LOW = 0.25       # noisy partial window â€” raise edge, cut size
...
# bot's model probability diverges from the market price (market lags BTC) â€”
...
# bucket alone was -$64.55 at 47% WR â€” no favorite premium exists at taker
...
# (the "strat" key in bots/base_bot.py STRATEGY_SIGNAL_PROFILE â€” BUG #27
...
# (drift ~ 0) flow-only trades are allowed â€” they measured break-even.
DRIFT_VETO_MIN = 0.05
...
# 0.10-0.30 "mid" band â€” released to full trust by the step â€” was actually
...
# a cliff-edge free pass. DRIFT_VETO_MIN (0.05) is unchanged â€” contradicting
...
# graduated mid-band tax is required; 1.5Ã— left too much mid-drift noise.
FLOW_ONLY_EDGE_MULT_MAX = 1.35     # 2.0â†’1.35 (audit: noisy cvd/pm lanes kill-switched; graduated tax still applies but gentler)
# Full flow-only tax lifts only once |drift| reaches this. Recal 2026-08-07:
# moderate z (0.15â€“0.50) was anti-predictive for directionals; require stronger
# conviction before trusting flow/strat residual. Graduated tax below this.
FLOW_ONLY_DRIFT_FULL_TRUST = 0.25   # 0.45â†’0.25 (audit: moderate z at 0.25Â± is genuinely predictive now that noisy lanes are gone)
# Adaptive mom saturation scale (1m return â†’ soft_saturate): track live Ïƒ so
# high-vol tape does not treat every 0.2% candle as full signal (same disease
# as under-scaled drift). Off â†’ fixed MOM_SCALE_PRIOR.
MOM_ADAPTIVE_SCALE = True
MOM_SCALE_PRIOR = 0.002           # 0.2% 1m â†’ ~0.76 (historical p97)
MOM_SCALE_MIN = 0.0015
MOM_SCALE_MAX = 0.005
MOM_SCALE_VOL_MULT = 1.35         # mom_scale â‰ˆ mult Â· Ïƒ_1m (Ïƒ_1m = Ïƒ_5m/âˆš5)

# --- Dead-zone gate (2026-07-21) â€” the single biggest live leak ---
...
# band with |drift| >= 0.30 still profited (+$30.10, 65.7% WR) â€” the validated
# "market lags drift" money â€” so the gate is drift-CONDITIONAL: a directional
...
DEAD_ZONE_DRIFT_MIN = 0.15          # 0.20â†’0.15 (audit: 0.10â€“0.20 band was profitable at 65.7% WR +$30.10)
# Quiet / range regimes: mid-band "market lags drift" at |drift| 0.10â€“0.20 was
# a major leak (2026-07-29 soak: 0.50â€“0.58 band âˆ’$35.9 at 48.5% WR under
# low_vol_range). Require stronger drift before allowing coin-flip mids.
DEAD_ZONE_QUIET_DRIFT_MIN = 0.18    # 0.30â†’0.18 (audit: 0.10â€“0.20 was profitable in quiet; raised floor still blocks sub-0.10 noise)
...
# |drift| 0.30â€“0.50 was the money zone (85% WR); |drift| â‰¥ 0.50 lost (41% WR).
# Do not hard-veto extreme drift â€” require the market to still LAG (side mid
...
# whenever the mid moved away from 0.5 â€” a structural underdog-fade that ran
...
# Full trust at lean â‰¥ 0.10 (validated market-lags-drift band). Re-tightened
...
# demanded |drift| >= 0.286 from the drift-pure meanrev profile â€” while the
...
MODEL_LEAN_MIN = 0.05  # live/base; paper data_gather_v1 â†’ 0.04
...
# gapped book) â€” a directional bot stands down. A REAL cross-book gap is the
...
# bankroll (compounding) â€” replacing the old flat 5-9.5%-of-max-position
...
# (db.get_kelly_fraction) and editable in the dashboard Settings tab â€”
...
# 8/15 for -$34, and avg loss size exceeded avg win size â€” an outsized "edge"
...
# 0.10 â†’ 0.08 after confidence-inversion soak: max-edge trades were the
...
# for edges â‰¤ this; diminishing returns between here and KELLY_EDGE_CAP.
...
# When enabled, each bot Kelly-sizes against bankroll Ã— weight instead of the
# full shared pool (N bots Ã— full bankroll oversubscribed correlated risk).
# Weights sum to 1; rebalance on timer and/or regime change. Editable in the
# dashboard Settings â†’ Portfolio Allocation card.
PORTFOLIO_ALLOCATION_ENABLED = True   # default on; Kelly sizes against bankroll Ã— weight
...
PORTFOLIO_MIN_TRADES = 6              # 5m tape: ~2â€“3 directional fills/hour
...
# Strategy-family correlation priors (tandem risk) when market-overlap Ï is thin
...
PORTFOLIO_MIN_WEIGHT = 0.0            # no forced floor â€” losers can go to 0
PORTFOLIO_MAX_WEIGHT = 0.50           # hard cap even for proven winners
# Until live edge is proven (nâ‰¥EDGE_PROVEN_MIN_N and expectancy>0), no bot
...
# After nâ‰¥NEG_EXP_MIN_N with expectancy<0: strip manual floors and zero auto
...
PORTFOLIO_CORR_MIN_OVERLAP = 8        # shared markets needed to estimate Ï
PORTFOLIO_COLD_START_SCORE = 0.05     # tiny score for bots under sample floor
PORTFOLIO_LOSER_SCORE = 0.0           # ready bots with neg expectancy â†’ zero weight
...
PORTFOLIO_REGIME_REBALANCE_MIN_DWELL_SEC = 600.0  # 300â†’600 (audit: boundary chatter flips every few min; 10min dwell)
...
# None â†’ use LIVE_MAX_DAILY_LOSS_* in live mode, paper defaults above in paper
...
RISK_UNDERPERFORM_PAUSE_PNL = -30.0  # was âˆ’40; catch mom-class bleed sooner
...
RISK_UNDERPERFORM_GRADUATED_TIERS = (
    (-20.0, 0.75),   # at âˆ’$20 â†’ size Ã—0.75
    (-30.0, 0.50),   # at âˆ’$30 â†’ size Ã—0.50
    (-40.0, 0.25),   # at âˆ’$40 â†’ size Ã—0.25 (never zero)
)
...
# Bound once under LOG_DIR after that path is resolved (see below).
...
# Deprecated alias â€” same semantic as HOUR_ET (kept so old env/docs still resolve).
...
# Core-lane tuner: notify when applied |Î”w| â‰¥ this (one step is 0.05)
...
# the bot read-only â€” the token then only exposes reports, never trading state.
TELEGRAM_COMMANDS_CONTROL_ENABLED = True
# Sender allowlist. Empty tuple = only accept messages whose sender id equals
# the chat id â€” i.e. the operator's own PRIVATE chat. Without this, pointing
...
# moment that has passed â€” refuse it rather than fire it hours late. Reports
...
# double-counted price. When False: bias contributes 0 AND record_outcome /
# bot_learning writes are skipped (enable explicitly in tests/redesign).
# See spec R5.
...
# groups had 3+ bots in the 2026-07-17 run) â€” per-bot Kelly doesn't know the
...
# cost by max(corr(self, peer), EXPOSURE_CORR_FLOOR). Ïâ‰ˆ1 bots (momentum/
# phantom/hybrid) almost fully share the same budget slot so tandem fills
# cannot 4Ã— one candle. Floor keeps partially-independent bots from free-riding.
...
# Overnight 2026-08-20: unique-market +$3.84 vs multi-bot âˆ’$36. Model
# "edges" of 5â€“23Â¢ always cleared the 3.5Â¢ pile-in bar. One directional
# thesis per (market, side).
MARKET_SIDE_MAX_BOTS = 1  # profit-mode: one directional per side (was 0 paper-eval)              # 0 = unlimited (paper-eval sample size)
...
# side), a new bot must clear a higher edge bar â€” unless confidence is very
...
PILEIN_EV_MIN_EDGE = 0.035        # absolute min edge when â‰¥1 peer is open
# quality_confidence is a 0â€“0.95 *structure* score, not P(win). Soak
# 2026-08-19: tandem mom/hybrid/sniper routinely printed 0.82â€“0.91 and
# skipped this bar â€” unique-market +$24 vs multi-bot âˆ’$12. Sit above the
...
# --- High-vol favorite tax (soak 2026-08-24: 50â€“58Â¢ Ã— high_vol_trend âˆ’$15) ---
# Chosen-side MID â‰¥ 0.52 in high_vol_trend: raise min_edge and cut size.
# Cheap lag (42â€“50Â¢) and cheap NO are untouched.
...
# Kalshi NO paper tax stub â€” 1.0 = off (do not tighten dual-gate). Next soak
# can raise this without a rewrite if Kalshi NO stays ~35% WR.
KALSHI_NO_EDGE_MULT = 1.0
HIGH_VOL_FAVORITE_SIZE_MULT = 0.60
# Mom sign fights drift: raise min_edge so Î¦-YES at 58Â¢ with mom=âˆ’0.83 dies.
...
LAG_JUSTIFIED_MIN_EDGE = 0.02      # implied âˆ’ mid âˆ’ fee must clear this
...
STRAT_FIGHT_TRUST_DAMP = 0.55  # trust_eff *= 1 âˆ’ damp * uncertainty_share

# Sticky risk taper: once reduced, stay reduced until DD recovers below
# start_frac * recovery_ratio (stops reducedâ†”active flapping at the boundary).
...
#   passive_mid â€” min(mid, best_ask âˆ’ tick): prefer resting maker (fee 0)
#   join_bid    â€” best_bid (true join; lowest fill rate, pure maker)
#   aggressive  â€” best_ask (marketable limit; still taker fee when it crosses)
ORDER_STYLE = "limit"
...
GATE_TUNE_LOOSEN_EDGE = 0.02    # +2Â¢/share hyp P&L after taker fee
GATE_TUNE_APPLY_COOLDOWN_SEC = 86400  # one step per knob per day if apply is on
# Dual-gate hyp is scored on the 50â€“58Â¢ band separately from 90Â¢ locks.
...
# Confirm-apply uses only *earned cheap* combos â€” it does not loosen
...
REGIME_ADAPT_BAD_WR = 0.48       # at/below â†’ size toward MIN
REGIME_ADAPT_GOOD_WR = 0.62      # at/above â†’ size toward MAX
REGIME_ADAPT_SIZE_MIN = 0.85     # was 0.35 â€” keep frequency, mild taper only
REGIME_ADAPT_SIZE_MAX = 1.15
REGIME_ADAPT_CACHE_SEC = 15.0
# Continuous edge tax from blended strategyÃ—regime WR (data-driven)
REGIME_ADAPT_CONT_EDGE_MAX = 1.55   # mult at wr â‰ª BAD
REGIME_ADAPT_CONT_EDGE_MIN = 0.95   # mult at wr â‰« GOOD
...
REGIME_HARD_SKIP_MIN_TRADES = 80   # was 20 â€” avoid thin-sample freezes
...
# (2026-08: 229 trades, 50% WR, âˆ’$37; with low_vol_trend âˆ’$49).
...
# Cap decision_events growth (maintenance path only â€” not the 1s trader).
# Keep newest MAX_ROWS; also drop rows older than RETAIN_DAYS when set.
DECISION_EVENTS_MAX_ROWS = 50000
DECISION_EVENTS_RETAIN_DAYS = 21
DECISION_EVENTS_PRUNE_INTERVAL_SEC = 3600  # evolution-host cadence
...
# CF step is scaled by HYBRID_META_CF_ETA_SCALE so one skip â‰  one real trade.
HYBRID_META_CF_ENABLED = False         # fills only â€” skip CF overfit (2.5Ã— mom)
...
SESSION_SKIP_WINDOWS_ET = [
    "09:30-10:15",   # NYSE open â€” highest direction-flip count per window
    "15:45-16:15",   # NYSE close â€” second flip spike
]

# --- Clean-tick guard (signals/clean_tick.py) ---
# Reject implausible single-tick price jumps and drop the first (possibly stale)
# tick from a fresh token. A real Polymarket YES mid does not move >15Â¢ between
# two reads a second apart â€” that is bad data, not a reprice.
...
# read is already current â€” dropping it would just blank a new market's price
...
COPYTRADING_COPY_NO_BETS = False        # Copy NO bets â€” data shows NO side loses money, skip by default
...
#   - pos monitor : ~1.0s SL/TP exit loop (warm-store first; see arena/position_monitor.py)
...
POSITION_MONITOR_INTERVAL_SEC = 1.0  # SL/TP monitor poll (prefer warm mids)
...
# all trading-decision inputs â€” YES+NO prices, both books, OBI, CVD, PM
# momentum â€” stay <=1s fresh. Lower = fresher but more HTTPS/sec to the CLOB.
...
# global weights are 0 â€” frees the 1s cycle for books. When a lane is live
...
# Dual-venue warmer (PM + Kalshi) can exceed 3s under book/BRTI load;
# 5.5s keeps the trader from false-skipping a still-usable warm snapshot.
WARM_MAX_AGE_SEC = 5.5
...
# feature table for the learned bias) â€” data that only changes when a trade
...
# guards only now â€” the warmer is effectively their sole caller and refreshes
...
# spend so a trade always clears this (5 shares Ã— price Ã— buffer) â€” otherwise
# small-edge bets get rejected 'below_min_size' and never fill.
POLYMARKET_MIN_SHARES = 5

# --- Backtesting (backtest/ package â€” offline replay only) ---
...
MAKER_UPCOMING_WINDOW_SEC = 1200  # â‰¤N seconds in the future the maker section is
...
# Logging â€” override with ARENA_LOG_DIR for Docker / non-default layouts.
...
# ---------------------------------------------------------------------------
# Environment overrides (opt-in) â€” operational knobs only
# ---------------------------------------------------------------------------
...
# from slice D). Deliberately EXCLUDED: TRADING_MODE (must start paper â€” flip it
...
STRATEGY_LAB_ENABLED = _env_bool("STRATEGY_LAB_ENABLED", STRATEGY_LAB_ENABLED)
STRATEGY_LAB_AUTO_PROMOTE = _env_bool("STRATEGY_LAB_AUTO_PROMOTE", STRATEGY_LAB_AUTO_PROMOTE)
STRATEGY_LAB_PAPER_SLOTS = int(_env_num(
    "STRATEGY_LAB_PAPER_SLOTS", STRATEGY_LAB_PAPER_SLOTS, int
))
STRATEGY_LAB_CYCLE_INTERVAL_SEC = _env_num(
    "STRATEGY_LAB_CYCLE_INTERVAL_SEC", STRATEGY_LAB_CYCLE_INTERVAL_SEC, float
)
STRATEGY_LAB_MAX_OPEN = int(_env_num(
    "STRATEGY_LAB_MAX_OPEN", STRATEGY_LAB_MAX_OPEN, int
))
STRATEGY_LAB_MAX_NEW_PER_TICK = int(_env_num(
    "STRATEGY_LAB_MAX_NEW_PER_TICK", STRATEGY_LAB_MAX_NEW_PER_TICK, int
))
STRATEGY_LAB_BACKTEST_MIN_PNL = _env_num(
    "STRATEGY_LAB_BACKTEST_MIN_PNL", STRATEGY_LAB_BACKTEST_MIN_PNL, float
)
STRATEGY_LAB_BACKTEST_MIN_TRADES = int(_env_num(
    "STRATEGY_LAB_BACKTEST_MIN_TRADES", STRATEGY_LAB_BACKTEST_MIN_TRADES, int
))
STRATEGY_LAB_PROMOTE_MIN_TRADES = int(_env_num(
    "STRATEGY_LAB_PROMOTE_MIN_TRADES", STRATEGY_LAB_PROMOTE_MIN_TRADES, int
))
STRATEGY_LAB_PROMOTE_MIN_DAYS = int(_env_num(
    "STRATEGY_LAB_PROMOTE_MIN_DAYS", STRATEGY_LAB_PROMOTE_MIN_DAYS, int
))
STRATEGY_LAB_PROMOTE_TRADE_FLOOR = int(_env_num(
    "STRATEGY_LAB_PROMOTE_TRADE_FLOOR", STRATEGY_LAB_PROMOTE_TRADE_FLOOR, int
))
_lab_llm = os.environ.get("STRATEGY_LAB_LLM_PROVIDER")
if _lab_llm and _lab_llm.strip():
    STRATEGY_LAB_LLM_PROVIDER = _lab_llm.strip().lower()
...
# clear message â€” never mid-session against real (or simulated) money. This does
...
def get_max_daily_loss_per_bot():
    ...
    Paper uses RISK_PAPER_BOT_DAILY_LOSS (same floor as the risk engine) so the
    engine-off legacy path is never uncapped (historically PAPER_*=999999).
    """
    if TRADING_MODE == "live":
        return LIVE_MAX_DAILY_LOSS_PER_BOT
    return float(RISK_PAPER_BOT_DAILY_LOSS)


def get_max_daily_loss_total():
    """Get max total daily loss based on current mode.

    Paper uses RISK_PAPER_PORTFOLIO_DAILY_LOSS - never the old 999999 uncapped.
    """
    if TRADING_MODE == "live":
        return LIVE_MAX_DAILY_LOSS_TOTAL
    return float(RISK_PAPER_PORTFOLIO_DAILY_LOSS)


def get_venue():
    """Trading venue â€” always Polymarket now (paper simulates against its books)."""
    return "polymarket"
